...
| Enable OpenID Search | Enable searching for the user using the `preferred_username` parameter from an incoming OpenID request (e.g. the EntraID external-authentication method). |
| OpenID Search Attribute | The user attribute used to search for the user with the `preferred_username` parameter from an incoming OpenID request. |
